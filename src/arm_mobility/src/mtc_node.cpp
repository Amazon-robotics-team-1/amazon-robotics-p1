...
mtc::Task MTCTaskNode::createTask()
{
  ...
  // In fact, `task` itself is a SerialContainer by default.
  {
...

    {
      ...
      stage->setMinMaxDistance(0, 0.15);  // changed min from 0.1 to 0 here.
  ...
  ---- *               Generate Grasp Pose                *
    ...
    { 
      ...
      wrapper->setMaxIKSolutions(15);
      wrapper->setMinSolutionDistance(0.7);
...

    {
      ...
      stage->properties().configureInitFrom(mtc::Stage::PARENT, { "group" });
      stage->setMinMaxDistance(0, 0.3); // changed min from 0.1 to 0 here.
  ...
  //   {
  //     auto stage = std::make_unique<mtc::stages::MoveRelative>("retreat", sampling_planner);
  //     stage->properties().configureInitFrom(mtc::Stage::PARENT, { "group" });
